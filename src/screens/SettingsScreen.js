...
import { Ionicons, FontAwesome, Foundation } from "react-native-vector-icons";
...
export class SettingsScreen extends React.Component {
  ...
  calculateText(key, values0, values1) {
    ...
    if (key === "Age") {
      ...
      return same
        ? options[1][leftIndex]
        : `${options[1][leftIndex]}-${options[1][rightIndex]}`;
    } else if (key === "Shabbat Observance") {
      return same
        ? options[2][leftIndex]
        : `${options[2][leftIndex]}-${options[2][rightIndex]}`;
    } else if (key === "Kashrut Observance") {
      return same
        ? options[3][leftIndex]
        : `${options[3][leftIndex]}-${options[3][rightIndex]}`;
    }
  }
  ...
  changeValue = (values, category) => {
    let finalCategory =
      category === "Denomination"
        ...
        : category === "Shabbat Observance"
        ? "shabbatPreference"
        : category === "Kashrut Observance"
        ? "kashrutPreference"
        : category === "Age"
        ? "agePreference"
        : category === "Distance"
    ...
    this.props.dispatch(
      updateUserInfo("preferences", finalCategory, finalValue)
    );
  };

  generateFilters() {
    let preferences = [
      { Denomination: this.props.denominationPreference },
      { "Shabbat Observance": this.props.shabbatPreference },
      { "Kashrut Observance": this.props.kashrutPreference },
      { Age: this.props.agePreference }
    ];
  ...
  render() {
    let femaleIcon = <FontAwesome name='female' size={25} color='black' />;

    let maleIcon = <FontAwesome name='male' size={25} color='black' />;

    let bothIcon = <Foundation name='male-female' size={25} color='black' />;
    ...
    return (
      <ScrollView style={styles.settingsContainer}>
        <View style={styles.dividerContainer}>
          <Text style={{ fontWeight: "bold" }}>Discoverable</Text>

          <Switch
            trackColor={{ true: "pink" }}
            onValueChange={() =>
              this.changeValue(!this.props.discoverable, "discoverable")
            }
        ...
        <View style={styles.filterContainer}>
          ...
          <Text>{this.props.distancePreference}</Text>
...
const mapStateToProps = state => {
  return {
    denominationPreference:
      state.userInfo.user.preferences.denominationPreference,
