...
export class ChooseProfilePictureScreen extends Component {
  ...
  askCameraPermissionsAsync = async () => {
    ...
    if (cameraPermission.status === "denied") {
      ...
      this.setState({
        permissionsError:
          "Oops! Looks like you haven't granted Basheret permission to your Camera. Please go into your settings and change that so you can take a picture for your profile!"
      });
    } else {
      console.log("2");
  ...
  askCameraRollPermissionsAsync = async () => {
    let cameraRollPermission = await Permissions.askAsync(
    ...
    if (cameraRollPermission.status === "denied") {
      ...
      this.setState({
        permissionsError:
          "Oops! Looks like you haven't granted Basheret permission to your Camera Roll. Please go into your settings and change that so you can upload a picture for your profile!"
      });
    } else {
      console.log("2");
  ...
  showImage() {
    if (this.props.profilePhoto) {
      ...
      return <MaterialIcons name='account-circle' size={280} color='#9fa4ad' />;
    }
  }
      ...
      return (
        <View>
          ...
          <Text style={styles.textLightStyle}>
            We ask, in the hopes of the broadest possible community, that all
            pictures for both men and women are Tzniut
          </Text>
        </View>
      );
    }
        ...
        <View style={styles.containerStyle}>
          ...
          <View style={styles.photoContainerStyle}>
            ...
            {this.state.permissionsError && (
              <Text>{this.state.permissionsError}</Text>
            )}
          </View>

          <View style={styles.textContainerStyle}>{this.showEdit()}</View>

          <View style={styles.buttonContainerStyle}>{this.showButton()}</View>
        </View>
      </SafeAreaView>
    );
  }
